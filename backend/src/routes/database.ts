...
router.get("/stats", authMiddleware, async (_req, res) => {
  try {
    ...
    const [agentsResult, postsResult, commentsResult, submoltsResult] =
      await Promise.all([
...
router.post("/init", async (_req, res) => {
  try {
    ...
    res.json({
      success: true,
      message:
        "Schema file ready. Copy the SQL from database/schema.sql and run it in your Supabase SQL editor.",
      schemaPath,
      instructions: [
